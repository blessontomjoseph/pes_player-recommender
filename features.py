...
def pos_val(team_data, unique_position):
    ...
    """
    pos_data = team_data.loc[team_data['registered_position']
                             == unique_position]
...
def feature_eng(data, new_features):
    ...
    for team in teams:
        ...
        team_data = data.loc[data['team_name']
                             == team]  # data for a unique team
        ...
        for player in player_dict:
            new_features['Team'][new_features['name'] == player] = np.linalg.norm(
                team_dict.get(team)-player_dict.get(player))

            for position in positional_dict:
                new_features[position][new_features['name'] == player] = np.linalg.norm(
                    positional_dict.get(position)-player_dict.get(player))
